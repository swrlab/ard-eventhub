...
```mermaid
flowchart LR
    P1["broadcaster<br/>playout systems"]

    subgraph GCP["GCP — legacy, best-effort"]
        EI["eventhub-ingest<br/>HTTPS auth"]
        MQ["MQTT broker"]
        PS["Cloud Pub/Sub<br/>until step 13"]
        EI --> MQ
        EI -.-> PS
    end

    subgraph CN["ARD CN — SWR · zones BAD, STG, MNZ"]
        NATS["NATS + JetStream<br/>MQTT gateway<br/>RAFT across the three zones"]
        EC["eventhub-connect<br/>validation · plugin routing · operator UI"]
        BR["eventhub-bridge"]
        NATS <-->|"validate · fan out"| EC
        BR -->|"inbox/"| NATS
    end
