...
4. **Plugin publishing feature flag in ingest.** `INGEST_PUBLISH_PLUGINS` must be the exact string
   `true` to dispatch DTS / Radioplayer jobs to the internal Pub/Sub topic. Anything else (unset,
   `1`, `TRUE`, `false`) leaves dispatch off. Distinct from the existing per-event
   `plugins[].isDeactivated`, which is a publisher's choice about one event; this one decides
   whether GCP dispatches plugin targets at all. A process restart picks up a change. Step 16 flips
   this when the CN adapters take over.
