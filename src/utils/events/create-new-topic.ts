 ...
 * @param user - Authenticated user creating the topic
 * @returns Topic metadata attached to the service
 */
export const createNewTopic = async (service: EventhubService, user: AuthUser) => {
	if (!user.email) {
		logger.notice({ message: 'user not found', source, data: { service } })
		throw new Error('User not found')
	}

	// check if topic is present
	if (!service.topic) {
		logger.notice({ message: 'topic not found', source, data: { service } })
	...
	if (!publisher) {
		logger.notice({ message: 'publisher not found', source, data: { service } })
	...
	if (result?.name?.includes(service.topic.name)) {
		...
		logger.notice({ message: `topic created > ${service.topic.name}`, source, data: { service, result } })
	} else {
		// update api result that topic was not created
		service.topic.status = 'TOPIC_NOT_CREATED'

		logger.error({ message: `failed creating topic > ${service.topic.name}`, source, data: { service, result } })
	}
